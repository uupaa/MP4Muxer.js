...
function MP4Muxer_mux(nalUnitObjectArray, // @arg NALUnitObjectArray - [NALUnitObject, ...]
                      options) {          // @arg Object = {} - { audioDuration, optimize }
...
//{@dev
    if (VERIFY) {
        ...
        $valid($keys(options,            "audioDuration|optimize"), MP4Muxer_mux, "options");
        if (options) {
...
//}@dev
    ...
    var samples         = _createSamples(nalUnitObjectArray, optimize); // NALUnit to Samples ([AccessUnit, ...])
