...
function MP4Muxer_mux(nalUnitObjectArray, // @arg NALUnitObjectArray - [NALUnitObject, ...]
                      options) {          // @arg Object = {} - { audioDuration }
...
//{@dev
    if (VERIFY) {
        ...
        $valid($keys(options,            "audioDuration"),      MP4Muxer_mux, "options");
        if (options) {
            $valid($type(options.audioDuration, "Number"),      MP4Muxer_mux, "options.audioDuration");
        }
    }
//}@dev
