...
function MP4Muxer_mux(nalUnitObjectArray, // @arg NALUnitObjectArray - [NALUnitObject, ...]
                      options) {          // @arg Object = {} - { audioDuration, parameterSet, optimize }
                                          // @options.audioDuration Number = 0.0
                                          // @options.parameterSet Object = null - latest ParameterSet
                                          // @options.optimize Boolean = false
...
//{@dev
    if (VERIFY) {
        ...
        $valid($keys(options,            "audioDuration|parameterSet|optimize"), MP4Muxer_mux, "options");
        if (options) {
            ...
            $valid($type(options.optimize,      "Boolean|omit"), MP4Muxer_mux, "options.optimize");
        }
    }
//}@dev
    ...
    var samples         = _createSamples(nalUnitObjectArray, parameterSet, optimize); // NALUnit to Samples ([AccessUnit, ...])
...
//}@dev
    ...
    var audioDuration   = options["audioDuration"] || 0;
